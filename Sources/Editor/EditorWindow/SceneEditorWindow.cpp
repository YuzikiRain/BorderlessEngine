...
namespace BorderlessEditor {
    ...
    void SceneEditorWindow::Draw() {
        ...
        ImGui::LabelText("camera Position", "%.2f,%.2f,%.2f", cameraTransform.Position.x, cameraTransform.Position.y,
                         cameraTransform.Position.z);
        ImGui::LabelText("euler", "%.2f,%.2f,%.2f", cameraTransform.GetEuler().x, cameraTransform.GetEuler().y,
                         cameraTransform.GetEuler().z);
        ImGui::LabelText("Front", "%.2f,%.2f,%.2f", cameraTransform.Front.x, cameraTransform.Front.y,
                         cameraTransform.Front.z);
        ImGui::LabelText("Right", "%.2f,%.2f,%.2f", cameraTransform.Right.x, cameraTransform.Right.y,
        ...
        for (vector<BorderlessEngine::GameObject *>::iterator it = objs.begin(); it != objs.end(); it++) {
            ...
            glm::mat4 view = glm::lookAt(cameraTransform.Position,
                                         cameraTransform.Position + cameraTransform.Front * 10.0f, cameraTransform.Up);
            // glm::mat4 view = cameraTransform.GetMatrix();
            material.shader->setMatrix4("view", view);
            glm::mat4 projection = glm::perspective(glm::radians(camera.fov), wsize.x / wsize.y, camera.nearPlane,
                                                    camera.farPlane);
